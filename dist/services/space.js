...
class SpaceService {
    async init() {
        await storage_js_1.storage.init();
    }
    // ─────────────────────────────────────────────────────────────────
    // Space CRUD
    // ─────────────────────────────────────────────────────────────────
    async createSpace(request) {
        ...
        await storage_js_1.storage.createSpace(spaceId, metadata, createEmptyProfile(), createEmptyFacts(), createEmptyNotes(), createEmptyTimeline());
        return metadata;
    }
    ...
    async updateSpace(spaceId, request) {
        const metadata = await storage_js_1.storage.readMetadata(spaceId);
        const timestamp = now();
        const updated = {
            ...metadata,
            updatedAt: timestamp,
        ...
        await storage_js_1.storage.writeMetadata(spaceId, updated);
        return updated;
    }
    async deleteSpace(spaceId) {
        await storage_js_1.storage.deleteSpace(spaceId);
    }
    async listSpaces() {
        const spaceIds = await storage_js_1.storage.listSpaceIds();
        const spaces = [];
        for (const id of spaceIds) {
            try {
                const [metadata, facts, notes] = await Promise.all([
                    storage_js_1.storage.readMetadata(id),
                    storage_js_1.storage.readFacts(id),
                    storage_js_1.storage.readNotes(id),
                ]);
                spaces.push({
                    id: metadata.id,
                    name: metadata.name,
                ...
                if (error instanceof storage_js_1.StorageError && error.code === 'NOT_FOUND') {
                    continue;
                }
                throw error;
            }
    ...
    // Facts
    // ─────────────────────────────────────────────────────────────────
    async addFact(spaceId, request) {
        const facts = await storage_js_1.storage.readFacts(spaceId);
        const timestamp = now();
        const fact = {
            id: (0, uuid_1.v4)(),
            createdAt: timestamp,
        ...
        await storage_js_1.storage.writeFacts(spaceId, facts);
    ...
    async updateFact(spaceId, factId, request) {
        const facts = await storage_js_1.storage.readFacts(spaceId);
        const index = facts.items.findIndex(f => f.id === factId);
        if (index === -1) {
            throw new storage_js_1.StorageError(`Fact not found: ${factId}`, 'NOT_FOUND');
        }
    ...
    async deleteFact(spaceId, factId) {
        const facts = await storage_js_1.storage.readFacts(spaceId);
        const index = facts.items.findIndex(f => f.id === factId);
        if (index === -1) {
            throw new storage_js_1.StorageError(`Fact not found: ${factId}`, 'NOT_FOUND');
    ...
    // Notes
    // ─────────────────────────────────────────────────────────────────
    async addNote(spaceId, request) {
        const notes = await storage_js_1.storage.readNotes(spaceId);
        const timestamp = now();
        const note = {
            id: (0, uuid_1.v4)(),
            createdAt: timestamp,
        ...
        await storage_js_1.storage.writeNotes(spaceId, notes);
        await this.addTimelineEntry(spaceId, {
    ...
    async updateNote(spaceId, noteId, request) {
        const notes = await storage_js_1.storage.readNotes(spaceId);
        const index = notes.items.findIndex(n => n.id === noteId);
        if (index === -1) {
            throw new storage_js_1.StorageError(`Note not found: ${noteId}`, 'NOT_FOUND');
        }
    ...
    async deleteNote(spaceId, noteId) {
        const notes = await storage_js_1.storage.readNotes(spaceId);
        const index = notes.items.findIndex(n => n.id === noteId);
        if (index === -1) {
            throw new storage_js_1.StorageError(`Note not found: ${noteId}`, 'NOT_FOUND');
    ...
    async getNotes(spaceId) {
        const notes = await storage_js_1.storage.readNotes(spaceId);
        return notes.items;
    }
    async promoteNoteToFact(spaceId, noteId, request) {
        const notes = await storage_js_1.storage.readNotes(spaceId);
    ...
    // Profile
    // ─────────────────────────────────────────────────────────────────
    async addProfileEntry(spaceId, request) {
        const profile = await storage_js_1.storage.readProfile(spaceId);
        const timestamp = now();
        const entry = {
            id: (0, uuid_1.v4)(),
            createdAt: timestamp,
        ...
        await storage_js_1.storage.writeProfile(spaceId, profile);
        await this.addTimelineEntry(spaceId, {
    ...
    async updateProfileEntry(spaceId, entryId, request) {
        const profile = await storage_js_1.storage.readProfile(spaceId);
        const index = profile.entries.findIndex(e => e.id === entryId);
        if (index === -1) {
            throw new storage_js_1.StorageError(`Profile entry not found: ${entryId}`, 'NOT_FOUND');
        }
    ...
    async deleteProfileEntry(spaceId, entryId) {
        const profile = await storage_js_1.storage.readProfile(spaceId);
        const index = profile.entries.findIndex(e => e.id === entryId);
        if (index === -1) {
            throw new storage_js_1.StorageError(`Profile entry not found: ${entryId}`, 'NOT_FOUND');
    ...
    // Timeline
    // ─────────────────────────────────────────────────────────────────
    async addTimelineEntry(spaceId, request) {
        const timeline = await storage_js_1.storage.readTimeline(spaceId);
        const timestamp = now();
        const entry = {
            id: (0, uuid_1.v4)(),
            createdAt: timestamp,
        ...
        await storage_js_1.storage.writeTimeline(spaceId, timeline);
        return entry;
    }
    async getTimeline(spaceId, limit = 50) {
        const timeline = await storage_js_1.storage.readTimeline(spaceId);
    ...
    async queryContext(request) {
        const [metadata, profile, facts, notes, timeline] = await Promise.all([
            storage_js_1.storage.readMetadata(request.spaceId),
            storage_js_1.storage.readProfile(request.spaceId),
            storage_js_1.storage.readFacts(request.spaceId),
            storage_js_1.storage.readNotes(request.spaceId),
            storage_js_1.storage.readTimeline(request.spaceId),
        ]);
        // Filter by categories if provided
        const categoryFilter = (item) => !request.categories?.length || request.categories.includes(item.category ?? '');
        // Filter by tags if provided
        const tagFilter = (item) => !request.tags?.length || item.tags.some(t => request.tags.includes(t));
        ...
        const profileSummary = profile.entries
            .filter(e => {
            if (e.validFrom && e.validFrom > currentTime)
                return false;
            if (e.validUntil && e.validUntil < currentTime)
                return false;
            return true;
        })
            .map(e => ({
